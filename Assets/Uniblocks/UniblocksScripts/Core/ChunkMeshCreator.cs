...
public class ChunkMeshCreator : MonoBehaviour {
	...
	private void CreateFace ( ushort voxel, Facing facing, ColliderType colliderType, int x, int y, int z ) { 
		...
		UVs.Add(new Vector2 (tUnit * tOffset.x, 			tUnit * tOffset.y + tUnit	)); 
		UVs.Add(new Vector2 (tUnit * tOffset.x + tUnit, 	tUnit * tOffset.y + tUnit	)); 
		UVs.Add(new Vector2 (tUnit * tOffset.x + tUnit,		tUnit * tOffset.y			));
		UVs.Add(new Vector2 (tUnit * tOffset.x, 			tUnit * tOffset.y			)); 
	...
	private void CreateCustomMesh (ushort voxel, int x, int y, int z, Mesh mesh) {
		...
		if (Engine.GenerateColliders) {
			...
			if ( colliderType == ColliderType.mesh ) {
				foreach (Vector3 vertex1 in mesh.vertices) {
