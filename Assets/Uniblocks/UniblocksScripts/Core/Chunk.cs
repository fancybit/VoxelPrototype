...
public class Chunk : MonoBehaviour {
	...
	public bool DisableMesh; // for chunks spawned with ChunkManager.SpawnChunk; if true, the chunk will not build a mesh
	...
	public void LateUpdate () {
		...
		if (FlaggedToRemove) {
			...
			if (Engine.SaveVoxelData) { // save data over time, destroy chunk when done
				if (ChunkManager.SavesThisFrame < ChunkManager.MaxChunkSaves) {
	...
	public void UpdateNeighbor ( Direction direction ) { // update the mesh for a specific neighbor chunk
		GetNeighbors();
		if (NeighborChunks[(int)direction] != null) {
			NeighborChunks[(int)direction].FlagToUpdate();
		}	
	}
	public int UpdateAllNeighbors () { // update the mesh for all neighbor chunks (unused)
		GetNeighbors();
		int updateCount = 0;
		foreach (var neighbor in NeighborChunks) {
			if (neighbor != null && neighbor.Fresh == false) { // we don't want to update fresh neighbors, because their voxels are already good.
